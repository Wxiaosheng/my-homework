...
# '''
# 	webapp 中有很多地方都要访问数据库
# 	访问数据库需要创建数据库连接、获取游标、执行SQL语句，最后还需要处理异常、清理资源。
...
# 		# 关闭 游标
# 		await cur.close()
# 		logging.info('rows returned: %s' % rs)
# 		return rs
...
# # ORM 映射基类
# # 注意: Model只是一个基类，如何将具体的子类如User的映射信息读取出来呢？答案就是通过metaclass：ModelMetaclass
# class Model(dict, mateclass=ModelMateClass):
...
# 	def getValueOrDefuate(self, key):
...
# 	@classMethod
# 	async def find(cls, pk): # ' find object by primary key. '
...
# async def save(self):
# 	args = list(map(self.getValueOrDefuate, self.__field__))
# 	args.append(self.getValueOrDefault(self.__primary_key__))
# 	rows = await self.execute(self.__insert__, args)
# 	if len(rows) != 1:
